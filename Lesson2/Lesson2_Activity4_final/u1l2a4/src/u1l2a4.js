 ...
 question_two.prototype = 
 {
  ...
  create : function()
  {
   //reg.modal = new gameModal(game);
    ...
    question_text_upper = game.add.text(125,11,'Find the share for a sub-group of 6 workers from each group.',style);
    question_text_upper.wordWrap = true;
 ...
 background = game.add.sprite(0,0,'bg','bg');
  ...
  var style4 = { font: "13px tahoma", fill: "ffff", boundsAlignH: "center", boundsAlignV: "middle" };
   ...
   for(var i=0;i<8;i++)
   {
    if(i<4)
    {  
    workers1[i] = game.add.sprite(60 + (i*65),130,'lesson2',worker_names[i]);
    workers1[i].scale.setTo(1,0.9);
     workers1[i].inputEnabled = true;
    workers1[i].input.enableDrag(true);
    workers1[i].originalPosition = workers1[i].position.clone();
    ...
    {
     workers1[i] = game.add.sprite(60 + ((i-4)*65),206,'lesson2',worker_names[i]);
    workers1[i].scale.setTo(1,0.85);
    workers1[i].inputEnabled = true;
    workers1[i].input.enableDrag(true);
    workers1[i].originalPosition = workers1[i].position.clone();
...
showModal8 : function()
{
  ...
  for(var i=0;i<2;i++)
  {
    ...
    //console.log(expected_sum);
    //console.log(i + 'sum' + plates1[i].sum);
    if(rect[4].sum == expected_sum_1 && rect[5].sum == expected_sum_2)
    {
     l=l+1;
    }
    if(rect[4].sum == 0 && rect[5].sum == 0)
    {
      k=k+1;
    }
  
    
    if(rect[2].numberof_workeringroup == 6 && rect[3].numberof_workeringroup == 2)
    {
     console.log('mmm' + m);
     m = m+ 1;
    }
